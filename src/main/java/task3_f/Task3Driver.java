...
public class Task3Driver extends Configured implements Tool {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
	    	...
	    	if (lineCount <0 ){
				...
				if(val.contains("REVISION ") && array.length == 7) {
					...
					lineCount =0; // As soon as we find the first revision then we know +14 lines each time.
	    	...
	    	else if (lineCount%14==0 && lineCount > 0){
	    		String val = value.toString();
				context.getCounter(Counters.TOTAL_LINES).increment(1);
				//split line
				String[] array = val.split(" ");
	...
	public int run(String[] args) throws Exception {
		...
		secondary_job.setJobName("Q3_Fast_job_2");
